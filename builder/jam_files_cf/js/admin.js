...
function Events04() { // app_builder.catalogs.sys_code_editor 
	...
	function select_editor(task, tag) {
		...
		if (!info.model) {
			// Create a Monaco model (like Ace EditSession)
			...
			create_info_tabs(task, tag);
			$(task.editor.getDomNode()).focus();  // equivalent focus
		}
		else {
			model = info.model;
		}
	
		// Attach model to editor (instead of setSession)
		task.editor.setModel(model);
	
		// Monaco doesn’t need $blockScrolling
		show_info_tabs(task, info);
		update_buttons(task);
		task.resize_elements(task);
		task.editor.focus();
